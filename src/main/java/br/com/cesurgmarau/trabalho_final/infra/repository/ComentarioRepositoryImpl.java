...
public class ComentarioRepositoryImpl implements ComentarioRepository {
    ...
    public void update(int id, Comentario comentario) {
        var query = """
                UPDATE comentario SET descricao :descricao, produto_id :produto_id, sentimento_id :sentimento_id, usuario_id :usuario_id
                WHERE id :id;
                """;
        entityManager.createNativeQuery(query, Comentario.class)
                .setParameter("descricao", comentario.getDescricao())
    ...
    public Comentario getById(int id) {
        var query = """
                SELECT descricao FROM comentario WHERE id = :id;
                """;
    ...
    public List<Comentario> getByProdutoId(int produto_Id) {
        var query = """
                SELECT descricao FROM comentario WHERE produto_Id = :produto_Id;
                """;
        return entityManager.createNativeQuery(query, Comentario.class)
    ...
    public List<Comentario> getByUsuarioId(int usuario_Id) {
        var query = """
                SELECT descricao FROM comentario WHERE usuario_id = :usuario_id;
                """;
        return entityManager.createNativeQuery(query, Comentario.class)
    ...
    public List<Comentario> getBySentimento(int sentimento_id) {
        var query = """
                SELECT descricao FROM comentario WHERE sentimento = :sentimento;
                """;
        return entityManager.createNativeQuery(query, Comentario.class)
